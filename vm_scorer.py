...
class Scorer:
    ...
    def get_operations_coverage(self) -> int:
        return sum(
            ...
            for operations_count in self.get_total_stats().values()
        )
...
def dump_tests_stat(stream: tp.TextIO, scorer: Scorer) -> None:
    ...
    data = [
        ...
        "\t"
        + "{}/{}".format(
            scorer.get_operations_coverage(), scorer.get_operations_count()
        ),
        "Operations by levels coverage:",
        "\n".join(
            "\t{}: {}/{}".format(
                level, count, scorer.get_level_operations_count()[level]
            )
